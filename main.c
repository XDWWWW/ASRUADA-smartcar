...
uint16 ServoPulse=1500;//舵机输出高电平时间，范围0-3000us，原理为输出高电平并用PIT1产生该值对应定时，定时结束转为低电平，PIT0定时20ms重新输出
   ...
   //====================================================================================
    ...
    {   
        ...
        {           
         ...
         if(set_speed>=40)
           set_speed=-40;
         MotorPower(set_speed);
         Motor2Power(set_speed);
          //gpio_turn(PTA17);//蜂鸣器
...
void MotorInit()
{
  ...
  //  PORT_cfg.h中有上述端口定义
   ftm_pwm_init(FTM0, FTM_CH2,10000,0);//初始化FTM0CH3，频率10k,占空比0。精度为1000u
   ftm_pwm_init(FTM0, FTM_CH3,10000,0);//初始化FTM0CH3，频率10k,占空比0。精度为1000u
...
static int ServMid=1500;//常量表示舵机中值 1500us 需根据实际情况修改
static int RightLimit=3000;//舵机右转极限 3000us 需根据实际情况修改
static int LeftLimit=500;//舵机左转极限 500us 需根据实际情况修改
...
//    
    ...
    if(kFar*kAverage>=0)
    {
        TurnError=(int)(TurnP*(kFar+2*kAverage+0.5*kNear)+0.5);
    }
    else
    {
        TurnError=(int)(TurnP*(0.3*kFar+2*kAverage+0.5*kNear)+0.5);
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////////////
